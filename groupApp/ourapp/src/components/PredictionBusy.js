...
const PredictionBusy = () => {
  ...
  useEffect(() => {
    fetchData()
  }, [dropoffLocationID, startMonth])

  const fetchData = async () => {
    try {
      const url = `/ml/passengerCount?location_id=${dropoffLocationID}&month_count=${startMonth}`
  ...
  const prepareChartData = () => {
    const labels = data.map((item) => item.prediction_period)
    const barData = data.map((item) => item.prediction)
